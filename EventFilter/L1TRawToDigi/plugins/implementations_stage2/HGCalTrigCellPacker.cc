...
#include "HGCalTrigCellPacker.h"

namespace l1t {
namespace stage2 {
   Blocks
   HGCalTrigCellPacker::pack(const edm::Event& event, const PackerTokens* toks)
   {

      edm::Handle<HGCalTriggerCellBxCollection> trigcells;
      event.getByToken(static_cast<const HGCalTokens*>(toks)->getHGCalTriggerCellToken(), trigcells);

      Blocks res;

      for (int i = trigcells->getFirstBX(); i <= trigcells->getLastBX(); ++i) {

        for (auto j = trigcells->begin(i); j != trigcells->end(i); ++j) {

          const uint32_t detId = j->detId();
          const double mipPt = j->mipPt();
          const uint32_t uncompressedCharge = j->uncompressedCharge();
          const uint32_t compressedCharge = j->compressedCharge();
...
DEFINE_L1T_PACKER(l1t::stage2::HGCalTrigCellPacker);
